/**
  destroying every table with same name for safety first
 */
DROP TABLE PRODUCT;/*produkt*/
DROP TABLE ORDER;/*contract*/
DROP TABLE REGISTER;/*rejstrik*/
DROP TABLE STORAGE_WORKER;/*skladnik*/
DROP TABLE MERCHANT;/*OBCHODNIK*/
DROP TABLE INVOICE;/*faktura*/


-----------------------CREATING TABLES-----------------------
CREATE TABLE "PRODUCT" (
     "ID_PRODUCT" INT GENERATED AS IDENTITY NOT NULL PRIMARY KEY,
     "NAME" VARCHAR(100) NOT NULL,
     "ORIGIN" VARCHAR(100) NOT NULL,
     "PRICE_IN_DOLLARS_FOR_KG" INT NOT NULL,
     "AGE_IN_WEEKS" INT NOT NULL,
     "IN_STOCK_IN_TONS" INT DEFAULT 0 NOT NULL,
     "STORAGE_WORKER_ID" INT DEFAULT NULL,
     CONSTRAINT "worker_id" FOREIGN KEY (STORAGE_WORKER_ID) REFERENCES "STORAGE_WORKER" ("ID") --product id je id v tabulce produkt
                ON DELETE SET NULL
     );

CREATE TABLE "REGISTER" (
     "ID" INT GENERATED AS IDENTITY NOT NULL PRIMARY KEY,
     "AMOUNT_IN_KG" INT NOT NULL,
     "PRODUCT_ID" INT DEFAULT NULL,
     CONSTRAINT "product_id" FOREIGN KEY (PRODUCT_ID) REFERENCES "PRODUCT" ("ID_PRODUCT") --product id je id v tabulce produkt
                ON DELETE SET NULL,
     "ORDER_ID" INT DEFAULT NULL,
     CONSTRAINT "order_id" FOREIGN KEY (ORDER_ID) REFERENCES "ORDER" ("ID_ORDER") --product id je id v tabulce produkt
                ON DELETE SET NULL
     );

CREATE TABLE "ORDER" (
     "ID_ORDER" INT GENERATED AS IDENTITY NOT NULL PRIMARY KEY,
     "TYPE" SMALLINT DEFAULT 0 NOT NULL
     CONSTRAINT "type" CHECK ( TYPE BETWEEN 0 AND 1), /*0 FOR PURCHASE 1 FOR SUPPLY*/
     "PRICE_IN_DOLLARS" INT NOT NULL,
     "AMOUNT" INT NOT NULL,
     "REGISTER_ID" INT DEFAULT NULL,
     CONSTRAINT "register_id" FOREIGN KEY (REGISTER_ID) REFERENCES "REGISTER" ("ID") --product id je id v tabulce produkt
                ON DELETE SET NULL
     );

CREATE TABLE "STORAGE_WORKER" (
     "ID" INT GENERATED AS IDENTITY NOT NULL PRIMARY KEY,
     "FIRST_NAME" VARCHAR(30) NOT NULL,
     "LAST_NAME" VARCHAR(30) NOT NULL,
     "PHONE_NUMBER" INT NOT NULL
     CONSTRAINT "phone_number" CHECK ( PHONE_NUMBER BETWEEN 111111111 AND 999999999),
     "EMAIL" VARCHAR(100) NOT NULL
     CHECK(REGEXP_LIKE(
			"EMAIL", '^[a-z]+[a-z0-9\.]*@[a-z0-9\.-]+\.[a-z]{2,}$', 'i'
		)),
     );
CREATE TABLE "MERCHANT" (
     "ID" INT GENERATED AS IDENTITY NOT NULL PRIMARY KEY,
     "FIRST_NAME" VARCHAR(30) NOT NULL,
     "LAST_NAME" VARCHAR(30) NOT NULL,
     "EMAIL" VARCHAR(100) NOT NULL
        CHECK(REGEXP_LIKE(
            "EMAIL", '^[a-z]+[a-z0-9\.]*@[a-z0-9\.-]+\.[a-z]{2,}$', 'i'
        )),
     "ADDRESS" VARCHAR(100) NOT NULL
     );

CREATE TABLE "INVOICE" (
     "id" INT GENERATED AS IDENTITY NOT NULL PRIMARY KEY,
     "DATE" DATE NOT NULL,
     "ORDER" INT DEFAULT NULL,
     CONSTRAINT "order_id" FOREIGN KEY (ORDER) REFERENCES "ORDER" ("ID_ORDER") --order id je id v tabulce zakazka
                ON DELETE SET NULL,
     "STORAGE_WORKER_ID" INT DEFAULT NULL,
     CONSTRAINT "worker_id" FOREIGN KEY (STORAGE_WORKER_ID) REFERENCES "STORAGE_WORKER" ("ID") --order id je id v tabulce zakazka
                ON DELETE SET NULL,
     "MERCHANT_ID" INT DEFAULT NULL,
     CONSTRAINT "merchant_id" FOREIGN KEY (MERCHANT_ID) REFERENCES "MERCHANT" ("ID") --order id je id v tabulce zakazka
                ON DELETE SET NULL
    );


CREATE TABLE "ORDER" (
    --TODO ONDRA IS NOT SURE :()
     );

-----------------------INSERTING DATA-----------------------

INSERT INTO "PRODUCT" ("NAME", "ORIGIN", "PRICE_IN_DOLLARS_FOR_KG", "AGE_IN_WEEKS", "IN_STOCK_IN_TONS")
VALUES("TOMATO", "SPAIN", 10, 3, 10);
INSERT INTO "PRODUCT" ("NAME", "ORIGIN", "PRICE_IN_DOLLARS_FOR_KG", "AGE_IN_WEEKS", "IN_STOCK_IN_TONS")
VALUES("TOMATO", "CZ", 12, 4, 20);

INSERT INTO "REGISTER" ("AMOUNT_IN_KG")
VALUES(20);
INSERT INTO "REGISTER" ("AMOUNT_IN_KG")
VALUES(35);

INSERT INTO "REGISTER" ("AMOUNT_IN_KG")
VALUES(1, 100);
INSERT INTO "REGISTER" ("AMOUNT_IN_KG")
VALUES(0, 250);